...
class ZigBeeDevice extends MeshDevice {
	...
	async _registerCapabilityListenerHandler(capabilitySetObj, capabilityId, value, opts) {
		...
		this.log(`Received command. _commandIsRunning = ${this._commandIsRunning}`);
		...
		if(!this._commandIsRunning) {
			...
			try {
				...
				this.log(`[${this.getName()}] [PERFORMING] Sending command to set ${capabilityId} -> ${value} (${commandId})`);
				await this._runCommand(cluster, commandId, parsedPayload, this);
			...
			this.log(`[${this.getName()}] [SKIPPED] Sending command to set ${capabilityId} -> ${value} (${commandId})`);
			return Promise.reject(new Error('Please wait 30 seconds between successive commands to the same device'));
		}
	}

	async _runCommand(cluster, commandId, parsedPayload, context) {
		cluster.do(commandId, parsedPayload, function(err, result) {
			context._commandIsRunning = false;
			context.log(`Error handling in runCommand. Error: ${err}. Result: ${result}`);
		});
	}
